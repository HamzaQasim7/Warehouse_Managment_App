...
class ReceiverUnassignedOrderScreen extends StatefulWidget {
  @override
  _ReceiverUnassignedOrderScreenState createState() => _ReceiverUnassignedOrderScreenState();
}

class _ReceiverUnassignedOrderScreenState extends State<ReceiverUnassignedOrderScreen> with TickerProviderStateMixin {
  //stores:---------------------------------------------------------------------
  ...
  void initState() {
    for (int i = 0; i < 10; i++) {
      ...
      String formattedArrivalTime = faker.randomGenerator.fromCharSet('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890', 10);
      list.add({
        'name': supplierName,
